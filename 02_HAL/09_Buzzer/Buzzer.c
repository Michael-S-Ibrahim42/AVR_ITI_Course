...
/* 
  ...
  Return     :   Error status of the fn, Error type is in "Buzzer.h" under "Error Type"
*/
Buzzer_enuStatus_t Buzzer_enuSetTones(u8 Copy_u8BuzzerNum, Buzzer_strToneBuff_t* Copy_pstrToneBuff)
{
  Buzzer_enuStatus_t Loc_enuError = Buzzer_enuStatus_Ok;
...
static void Buzzer_vidPlayTone(u8 Copy_u8BuzzerNum)
{
  ...
  {
    Pwm_SetPeriodAndDuty(Buzzer_astrBuzzers[Copy_u8BuzzerNum].PwmChannel,  50, 1000/(Buzzer_astrToneBuff[Copy_u8BuzzerNum]->Buffer[Buzzer_astrToneBuff[Copy_u8BuzzerNum]->Index]));
  }/* if */
  Buzzer_astrBuzzers[Copy_u8BuzzerNum].BuzzerCounter++;
  if(Buzzer_astrBuzzers[Copy_u8BuzzerNum].BuzzerCounter == Buzzer_astrToneBuff[Copy_u8BuzzerNum]->Duration)
  {
